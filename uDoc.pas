...
begin
  ...
  lvMerged.Invalidate;
  lMerged.Caption :=
    ' ×Ü¼Æ×ÔÉíÊ±¼ä: '+ FormatMs(SumSelf) +' ('+ FormatPercent(SumSelfPercent) + ')'
    + '   µ÷ÓÃ´ÎÊý: '+ Format('%.0n', [SumCalls * 1.0]);
end;
...
begin
  if (Item.Index >= 0) and (Item.Index < FListMerged.Count) then begin
    ...
    case TimeDisplay of
      ...
      raise Exception.Create('Î´ÖªÊ±¼äÏÔÊ¾Ñ¡Ïî.');
    end;
...
begin
  if tv.Selected <> nil then begin
    ...
    lInfoFileName.Caption := 'ÎÄ¼þ: '+ Inst.FileName;
    lInfo.Caption := '×ÔÉíÊ±¼ä: '+ FormatMs(Inst.SelfTime) + ' (' + FormatPercent(Inst.SelfPercent) + ')'
      + '   ÀÛ»ýÊ±¼ä: '+ FormatMs(Inst.CumTime) + ' (' + FormatPercent(Inst.CumPercent) + ')';
...
begin
  case TimeDisplay of
    ...
    raise Exception.Create('ÎÞ·¨Ê¶±ðµÄÊ±¼äÏÔÊ¾Ñ¡Ïî.');
  end;
end;
...
begin
  if Column.Index > Ord(High(TMergedInstancesSort)) then
    MessageDlg('²»Ö§³Ö¶Ô´ËÁÐ½øÐÐÅÅÐò.', mtInformation,
      [mbOK], 0)
  else
...
begin
  ...
  if not CacheGrind.SummaryExists then
    S := S + '¾¯¸æ£¡ ¸ÃÎÄ¼þ²»°üº¬ "summary:" ÐÐ,'
      + ' ÕâÍ¨³£ÒâÎ¶×ÅÎÄ¼þÎÞÐ§. ´Ó Xdebug 2.0.0 beta 1 ¿ªÊ¼'
      + ' µ±ÄúÔÚ½Å±¾ÖÐÊ¹ÓÃexit() »ò die() Ê±£¬¿ÉÄÜ»á·¢ÉúÕâÖÖÇé¿ö.'#13#10#13#10;
  S := S + '½âÎöÆ÷ÐÅÏ¢:'#13#10;
  S := S + '¶¨ÒåÐÐ: ' + Format('%.0n', [AInst.ParserLine * 1.0]) + #13#10;
  S := S + 'µ÷ÓÃÐÐ: ' + Format('%.0n', [AInst.ParserCallLine * 1.0]);
...
begin
  ...
  while FListMerged.IndexOf(AFunc) < 0 do begin
    if tv.Selected.Parent = nil then
      raise Exception.Create('ÔÚÕû¸öº¯ÊýÁÐ±íÖÐÎÞ·¨ÕÒµ½µ±Ç°º¯Êý (¿ÉÄÜÊÇ³ÌÐòÖÐµÄÒ»¸ö Bug).');
    SelectTreeNode(tv.Selected.Parent);
    // refresh lvMerged
    RefreshListMerged;
  end;
